    ...
    return (
        <a href={`post/${blog.slug}`} className="md:mx-2 md:mb-2 mb-2 sm:mb-2 p-2 sm:flex block bg-white rounded relative">
            ...
            <div className="py-3 px-3 sm:w-[60%] w-[100%]">
                ...
                <p className="my-4">
                    <a href={`/categories/${blog.category.replace(" ", "-")}`} className={`py-1 px-2 mr-2 rounded bg-fuchsia-100 text-fuchsia-600 text-xs hover:bg-fuchsia-500 hover:text-white`}>{blog.category}</a>
                    {
                    blog.tags.map((tag, i) => {

    ...
    return (
        <a href={`/post/${blog.slug}`} className="md:mx-2 md:mb-2 mb-2 sm:mb-2 p-2 sm:flex block bg-white rounded relative">
            ...
            <div className="py-3 px-3 sm:w-[60%] w-[100%]">
                <a href={`/categories/${blog.category}`} className={`py-2 rounded text-gray-500 text-xs font-semibold uppercase hover:text-fuchsia-500`}>{blog.category || <Skeleton />}</a>
                ...
                <p className="my-4">
                    {
                    (blog.tags.map((tag, i) => {
                        return <a href={`/tags/${tag.replace(" ", "-")}`} key={i} className={`py-1 px-2 mr-2 rounded bg-gray-100 text-gray-600 uppercase text-[10px] hover:bg-fuchsia-500 hover:text-white`}>{tag}</a>
                    })) || <Skeleton count={3} />
                }</p>

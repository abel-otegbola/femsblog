                ...
                <a href={`/categories/${post.category && post.category}`} className="flex uppercase text-[14px] mb-3">{post.category}</a>
                    ...
                    <p className="my-2 mb-8">
                        ...
                        post.tags && post.tags.map((tag, i) => {
                            return <a key={i} href={`/tags/${tag.replace(" ", "-")}`} className={`py-1 p-1 px-2 pb-2 mr-4 rounded uppercase text-xs bg-gray-100 hover:bg-fuchsia-500 hover:text-white`}>{tag}</a>
                        })
                    }</p>
